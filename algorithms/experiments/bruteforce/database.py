...
class Database(object):
  ...
  def __init__(self, sdow_database, searches_database=None):
    if not os.path.isfile(sdow_database):
      raise IOError('Specified SQLite file "{0}" does not exist.'.format(sdow_database))

    if searches_database is not None and not os.path.isfile(searches_database):
    ...
    self.searches_conn = None if searches_database is None else sqlite3.connect(searches_database, check_same_thread=False)

    self.sdow_cursor = self.sdow_conn.cursor()
    self.searches_cursor = None if searches_database is None else self.searches_conn.cursor()

    self.sdow_cursor.arraysize = 1000
    if searches_database is not None:
